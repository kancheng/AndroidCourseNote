package com.example.user.newclock;
...
public class MainActivity extends Activity {
    ...
    private int mHour,mMinutes,mSeconds, mmSeconds, syear, smon, sday;
    private String allymd;
...
//            clock.setText(mHour+" : "+mMinutes+" : "+mSeconds);
    ...
    class ClockThread extends Thread{
        public void run(){
            while(endClock){
                ...
                sday = mCalendar.get(Calendar.DATE);
                smon = mCalendar.get(Calendar.MONTH);
                syear = mCalendar.get(Calendar.YEAR);
    ...
    class MyView extends View{
        ...
        void draw(Canvas canvas, int x, int y, int width, int height, int h, int m, int s, int ms , String mname, String allyemoda) {
//            canvas.drawColor(Color.WHITE);
        ...
        protected void onDraw(Canvas canvas){
            ...
            String ma1, ma2, ma3, ma4;
            ma1 = "Taiwan, Taipei";
            ma2 = "England, London";
            ma3 = "Pakistan, Islamabad";
            ma4 = "Iraq, Baghdad";
            allymd = syear + " - " + smon + " - " + sday;
            /*  臺灣 臺北 */
            draw(canvas, 0, 0, width/2, height/2, mHour, mMinutes, mSeconds, mmSeconds, ma1, allymd);

            /* 英國 倫敦 */
            draw(canvas, width/2, 0, width/2, height/2, mHour+8, mMinutes, mSeconds, mmSeconds, ma2, allymd);

            /* 巴基斯坦  伊斯蘭瑪巴德 */
            draw(canvas, 0, height/2, width/2, height/2, mHour+3, mMinutes, mSeconds, mmSeconds, ma3, allymd);

            /* 伊拉克 巴格達 */
            draw(canvas, width/2, height/2, width/2, height/2, mHour+5, mMinutes, mSeconds, mmSeconds, ma4, allymd);
